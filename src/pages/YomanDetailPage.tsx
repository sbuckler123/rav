...
export default function YomanDetailPage() {
  const { id } = useParams<string>();
  const entry = id ? getYomanEntry(id) : undefined;
  ...
  const prevImage = useCallback(() => {
    ...
    setCurrentImageIndex(
      (prev) => (prev - 1 + entry.gallery.length) % entry.gallery.length
    );
  }, [entry]);
  ...
  const { prev, next } = getPrevNextYoman(entry.slug);
  const eventStyle = getEventTypeStyle(entry.eventType);
  const recentEntries = YOMAN_LIST.filter((e) => e.id !== entry.id).slice(0, 4);
  const sameTypeEntries = YOMAN_LIST.filter(
    (e) => e.eventType === entry.eventType && e.id !== entry.id
  ).slice(0, 3);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8 mt-6">
          ...
          <div className="space-y-8">
            ...
            <header>
              ...
              <div
                className="w-20 h-1 rounded-full bg-secondary mt-4"
                style={{ height: '4px' }}
                aria-hidden />
            ...
            <section>
              <div className="rounded-lg overflow-hidden border border-primary/20 mb-4">
                {entry.gallery.length > 0 && entry.gallery[0].url ?
                <button
                  type="button"
                ...
                <button
                  type="button"
                  onClick={() => openLightbox(0)}
                  className="block w-full aspect-video bg-[#E5E7EB] flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-secondary">
              ...
              <div className="flex gap-2 overflow-x-auto pb-2">
                ...
                <button
                  ...
                  className="shrink-0 w-20 h-14 rounded-md overflow-hidden border border-border hover:border-secondary transition-colors focus:outline-none focus:ring-2 focus:ring-secondary">
            ...
            <section className="prose prose-lg max-w-none">
              ...
              <p
                key={idx}
                className="font-sans text-base md:text-lg leading-[1.75] text-foreground mb-4">
            ...
            <section className="space-y-6">
              ...
              <blockquote
                ...
                style={{ backgroundColor: '#FAF8F2' }}>
                    ...
                    <footer className="font-sans text-sm font-normal text-muted-foreground">
                      — {q.author}
                    </footer>
                  </blockquote>
              )}
            ...
            <section>
                ...
                <div className="relative pr-6">
                  <div
                  className="absolute top-0 bottom-0 w-0.5 rounded-full bg-secondary"
                  style={{ right: '3px' }}
                  aria-hidden />
                  ...
                  <li key={idx} className="relative flex gap-4">
                        <span
                      className="absolute h-2.5 w-2.5 rounded-full bg-secondary border-2 border-white shrink-0"
                      style={{ right: '0', top: '0.25rem' }}
                      aria-hidden />
                        ...
                        <div className="mr-6">
                          ...
                          <span className="text-foreground text-[15px] mr-2">
                            {' '}
                            {item.description}
                          </span>
                        </div>
            ...
            <nav
              ...
              style={{ backgroundColor: '#FAF8F2' }}
              aria-label="ניווט בין רשומות">

              <div className="flex-1 min-w-0 text-left">
                {prev ?
                <Link
                  to={`/yoman/${prev.slug}`}
                  className="group block hover:bg-primary/5 rounded p-2 -m-2 transition-colors">

                    <span className="text-sm text-muted-foreground">
                      → הרשומה הקודמת
                    </span>
                    <span className="block font-semibold text-primary group-hover:text-secondary truncate">
                      {prev.title}
                    </span>
                  </Link> :
              ...
              <Link
                to="/yoman"
                className="text-primary font-medium hover:text-secondary text-sm shrink-0">
          ...
          <aside className="space-y-6">
              ...
              <ul className="space-y-3">
                ...
                <li key={e.id}>
                    ...
                    className="flex gap-3 rounded-lg p-2 hover:bg-[#F5F0E8] transition-colors group">
                      ...
                      <div className="h-12 w-16 shrink-0 rounded bg-[#E5E7EB] flex items-center justify-center">
                      ...
                      <div className="min-w-0">
                        ...
                        <span className="text-xs text-muted-foreground">
                          ...
                          style={{
                            backgroundColor: getEventTypeStyle(e.eventType).bg,
                            color: getEventTypeStyle(e.eventType).text
                          }}>
            ...
            {/* חזרה ליומן */}
            <div
              className="rounded-lg p-6 text-primary-foreground"
              style={{ backgroundColor: '#1B2A4A' }}>

              <p className="font-serif font-bold mb-3">כל היומן</p>
              <Button
                asChild
                size="sm"
