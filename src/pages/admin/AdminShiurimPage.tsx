...
function buildShiurim(records: any[], catMap: Record<string, string>, userRecords: any[]): Shiur[] {
...
export default function AdminShiurimPage() {
  ...
  function load() {
    setLoading(true);
    Promise.all([
      airtableFetch('שיעורים', {}, [{ field: 'תאריך', direction: 'desc' }]),
      airtableFetch('משתמשים'),
      fetchCategories('שיעורים'),
    ])
      .then(([shiurimData, usersData, cats]) => {
        setCategories(cats);
        const catMap = Object.fromEntries(cats.map((c: Category) => [c.id, c.name]));
        setShiurim(buildShiurim(shiurimData.records ?? [], catMap, usersData.records ?? []));
      })
          ...
          <div className="divide-y divide-border">
            {filtered.map(s => (
              <div key={s.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-muted/30 transition-colors">
                ...
                {s.category && (
                  <span className="text-xs text-muted-foreground hidden sm:block flex-shrink-0">{s.category}</span>
                )}
